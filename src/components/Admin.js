  ...
  function kmeans2() {
    ...
    for (let i = 0; i < group; i++) {
      for (let j = 0; j < data.length; j++) {
        ...
        distances.push({ itemId: j, clusterId: i, distance: distance });
      }
    }
    ...
    function recalculate() {
      ...
      for (let i = 0; i < group; i++) {
        for (let j = 0; j < data.length; j++) {
          ...
          newDistances.push({ itemId: j, clusterId: i, distance: distance });
        }
      }
      // sort distances in ascending order
    ...
    // for all centroids, create a group and assign users to that group
    for (let i = assigned; i < centroids.length; i++) {
      for (let j = 0; j < centroids[i].items.length; j++) {
        db.collection("users")
          .where("id", "==", centroids[i].items[j])
          .get()
          .then((querySnapshot) => {
            db.collection("groups").doc(i.toString()).set({ members: [] });
            querySnapshot.forEach((doc) => {
              //db.collection("users").doc(doc.id).update({ groupAssigned: true });
              //db.collection("users").doc(doc.id).set({ groupId: i }, { merge: true });
              //db.collection("groups").doc(i.toString()).get().then((doc) => {
              //let temp = doc.data().members
              //console.log(temp)
              //console.log(centroids[i].items[j])
              //temp.push(centroids[i].items[j])
              //db.collection("groups").doc(i.toString()).set({members: temp})
              //})
            });
          });
      }
      ...
      .then((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          ...
          if (count === 0) {
            ...
            db.collection("users")
              .doc(doc.id)
              .set({ groupId: assigned }, { merge: true });
            count++;
          } else if (count === 1) {
            db.collection("groups")
              .doc(assigned.toString())
